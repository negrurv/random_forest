...
def main():
    ...
    plt.plot(pnl_hist, label="Model Strategy (Value Betting with 5% Edge)", color="#1abc9c", linewidth=2.5)
    plt.plot(baseline_history, label="Baseline Strategy (Always Bet Favorite)", color="#e74c3c", linewidth=1.5, linestyle="--")
    plt.axhline(0, color="gray", linestyle=":", linewidth=1)
    plt.title("Cumulative Strategy PnL over Time (Post-2022 Backtest)", fontsize=14, fontweight="bold", pad=15)
    ...
    for i, val in enumerate(targets):
        if val == 1.0: o[i, 0] = 1.0
        elif val == 0.5: o[i, 1] = 1.0
        elif val == 0.0: o[i, 2] = 1.0
    ...
    plt.plot(pred_win_rates, actual_win_rates, marker="o", color="#3498db", label="Random Forest Model", linewidth=2)
    plt.title("Probability Calibration Curve (10% Bins)", fontsize=14, fontweight="bold", pad=15)
    ...
    if os.path.exists(artifact_dir):
        ...
        shutil.copy(cal_path, os.path.join(artifact_dir, "calibration_curve.png"))
        print("Successfully copied charts to artifacts folder.")
        
        # Also write the metrics to a small text file for quick viewing if needed
        metrics_path = os.path.join(artifact_dir, "quant_metrics.txt")
        with open(metrics_path, "w") as f:
            f.write(f"Total Bets Placed : {result['total_bets']}\n")
            f.write(f"Betting Win Rate  : {result['win_rate']*100:.2f}%\n")
            f.write(f"Betting ROI       : {result['roi']*100:+.2f}%\n")
            f.write(f"Betting PnL       : {result['total_pnl']:+.2f} units\n")
            f.write(f"Max Drawdown      : {max_drawdown:.2f} units\n")
            f.write(f"Annualized Sharpe : {sharpe_ratio:.4f}\n")
